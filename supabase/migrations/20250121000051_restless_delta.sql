...
DROP FUNCTION IF EXISTS update_user_profile_v2(uuid, text, text, text, text, text, text, jsonb, text, text, text, text, text, text, text, text, text);

-- Create updated function with proper handling of notes
...
BEGIN
  ...
  VALUES (
    ...
    p_marital_status,
    CASE 
      WHEN v_is_agency THEN COALESCE(p_internal_agency_notes, v_profile.internal_agency_notes)
      ELSE v_profile.internal_agency_notes
    END,
    COALESCE(p_client_to_agency_notes, v_profile.client_to_agency_notes),
    CASE 
      WHEN v_is_agency THEN COALESCE(p_agency_to_client_notes, v_profile.agency_to_client_notes)
      ELSE v_profile.agency_to_client_notes
    END
  )
  ...
  SET
    ...
    internal_agency_notes = EXCLUDED.internal_agency_notes,
    client_to_agency_notes = EXCLUDED.client_to_agency_notes,
    agency_to_client_notes = EXCLUDED.agency_to_client_notes,
    updated_at = now()
